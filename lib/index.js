...
exports.app = function() {
    ...
    app.require = function(filepath, options) {
        var root = exports.router(options);
        root.mount(app, "/");
        exports.mount(root, filepath, options);
        return app;
    };
...
exports.mount = function(parent, filepath, options) {
    ...
    if (filepath.endsWith(".js")) {
        ...
        if (basename.toLowerCase() == "index") {
            module(parent, options);
        }
        else {
            var mountpath = path.join(options.path, basename),
        ...
        js.forEach(file => {
            ...
            if (basename.toLowerCase() == "index") {
                ...
                var mountpath = path.join(options.path, (basename.toLowerCase() == "index" ? "" : basename)),
                    router = exports.router(options);
        ...
        other.forEach(file => {
            ...
            if (fs.lstatSync(path.join(filepath, file)).isDirectory()) {
                var mountpath = path.join(basepath, (file == "index" ? "" : file)),
                    router = exports.router(options);
...
exports.contentTypeOverride = function(req, res, next) {
    ...
    if (ext.length && ext != ".") {
        req.headers.accept = req.headers.Accept = mime.lookup(ext);
        res.type(ext);
        parse.pathname = parse.pathname.to(-1 * ext.length);
        req.url = url.format(parse);
    }
    else if (req.query.format) {
        req.headers.accept = req.headers.Accept = mime.lookup(req.query.format);
...
exports.generate = function(template, routers, options, cb) {
    ...
    if ([ "angular", "docs", "jquery", "node", "schema" ].indexOf(template) < 0) {
...
function Call(router) {
    ...
    this.format = this.formats = function() {
        ...
        Object.keys(inputs).forEach(function(key) {
            ...
            if (input.source) {
                ...
                value = input.source.include("params", "query", "body").map((source) => {
                    return req[source] ? req[source][key] : null;
                }).compact(true).first();
            }
            else {
            ...
            value = validateInput(key, input, value, err);
...
function validateInput(key, input, value, errors) {
    ...
    if (input.transform && value) {
        ...
        else if (input.transform == "camelcase") value = value.camelize(false);
        else if (input.transform == "titlecase") value = value.camelize();
        else errors.push(`${input.transform} is an invalid transformation.`);
    }

    if (input.required && (!value || value.toString().trim() == "")) {
        errors.push(input.error || `${key} cannot be missing.`);
    }
    else if (input.default && (!value || value.toString().trim() == "")) {
        value = input.default;
    }
    else if (!input.required && (!value || value.toString().trim() == "")) {
        value = "";
    }
    ...
    if (input.language && value) {
        ...
        if (languages.indexOf(input.language.toLowerCase()) >= 0) {
            if (!value["is" + input.language.capitalize()]()) {
                errors.push(`${key} is not written in the ${input.language.capitalize()} alphabet.`);
            }
        }
        else {
            errors.push(`${input.language} is not a recognized alphabet.`);
        }
    }
    ...
    if (input.type) {
        ...
        if (value && type == "text") value = value.toString();
            ...
            if (!validator.isMobilePhone(value, input.locale || "en-US")) {
            ...
            if (value.length != 2 || value.any(v => Object.isNaN(v))) {
                errors.push(input.error || `${key} is not a valid location.`);
            }
            else {
                value = { latitude: value[0], longitude: value[1] };
                if (value.latitude < -90 || value.latitude > 90) {
                    errors.push(input.error || `${key} does not have a valid latitude.`);
                }

                if (value.longitude < -180 || value.longitude > 180) {
                    errors.push(input.error || `${key} does not have a valid longitude.`);
                }
            }
        }
    }
    ...
    if (input.match) {
        ...
        else if (Object.isFunction(input.match)) {
