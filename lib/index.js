...
function Call(router) {
    ...
    this.defaultFormat = function(format) {
        me.defaultContentType = mime.lookup(format);
    };
        ...
        me.routes.forEach(function(route) {
            router[route.first()](
                ...
                (req, res, next) => {
                    ...
                    res.respond = (data, template) => { respond(req, res, me.acceptTypes, data, template); };
                    res.param = (name, defaultValue) => { return param(req, field, defaultValue); };
                    
                    if (!req.headers.Accept) {
                        req.headers.Accept = me.defaultContentType;
                    }
